...
using McdfLoader.Interop.GameModel;
    ...
    public unsafe Dictionary<string, List<ushort>>? GetSkeletonBoneIndices(GameObjectHandler handler)
    {
        ...
        try
        {
            for (int i = 0; i < chara->Skeleton->PartialSkeletonCount; i++)
            {
                ...
                if (handle->DefaultResourceHandle.FileName.Length > 1024) continue;
                var skeletonName = handle->DefaultResourceHandle.FileName.ToString();
